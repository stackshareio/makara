...
module ActiveRecord
  module ConnectionAdapters
    class MakaraAbstractAdapter < ::Makara::Proxy
      ...
      SQL_PRIMARY_MATCHERS          = [/\A\s*select.+for update\Z/i, /select.+lock in share mode\Z/i, /\A\s*select.+(nextval|currval|lastval|setval|get_lock|release_lock|pg_advisory_lock|pg_advisory_unlock|drop|truncate|create)\(/i].map(&:freeze).freeze
