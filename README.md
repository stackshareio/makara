...
[![Build Status](https://travis-ci.org/stackshareio/makara.png?branch=master)](https://travis-ci.org/taskrabbit/makara)
...
To handle persistence of context across requests in a Rack app, makara provides a middleware. It lays a cookie named `_mkra_stck` which contains the current context. If the next request is executed before the cookie expires, that given context will be used. If something occurs which naturally requires primary on the second request, the context is updated and stored again.

#### Stickiness Impact

When `sticky:true`, once a query as been sent to primary, all queries for the rest of the request will also be sent to primary.  In addition, the cookie described above will be set client side with an expiration defined by time at end of original request + `primary_ttl`.  As long as the cookie is valid, all requests will send queries to primary.

When `sticky:false`, only queries that need to go to primary will go there.  Subsequent read queries in the same request will go to replicas.
...
A context is local to the curent thread of execution. This will allow you to stick to primary safely in a single thread
in systems such as sidekiq, for instance.


#### Forcing Primary

If you need to force primary in your app then you can simply invoke stick_to_primary! on your connection:
...
It'll keep the proxy stuck to primary for the current request, and if `persist = true` (default), it'll be also stored in the context for subsequent requests, keeping the proxy stuck up to the duration of `primary_ttl` configured for the proxy.

#### Skipping the Stickiness

If you're using the `sticky: true` configuration and you find yourself in a situation where you need to write information through the proxy but you don't want the context to be stuck to primary, you should use a `without_sticking` block:

```ruby
proxy.without_sticking do
  # do my stuff that would normally cause the proxy to stick to primary
end
```
...
In general: Any `SELECT` statements will execute against your replica(s), anything else will go to primary.
...
[ConfigParser](https://github.com/taskrabbit/makara/blob/primary/lib/makara/config_parser.rb), see:
...
```ruby
# some third party creates a resource in your db, replica replication may not have completed yet
# ...
...
def handle_request_after_third_party_record_creation
  ...
  CreatedResourceClass.find(params[:id]) # will go to primary
end
```
